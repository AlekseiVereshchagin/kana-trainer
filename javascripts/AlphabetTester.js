function AlphabetTester(test_alphabet, check_alphabet) {
  ...
  //for (i = 0; i < test_alphabet.length; i++)
...
AlphabetTester.prototype.get_random_learning_index = function() {
  return this.LearningRate.prefix(
    Math.floor(Math.random() * this.LearningRate.sum())
  );
}
...
AlphabetTester.prototype.test_and_update = function(test_symbol, check_symbol) {
  var test_id = this.TestAlphabet.indexOf(test_symbol),
      check_id = this.CheckAlphabet.indexOf(check_symbol),
      res = test_id == check_id;
  if (res)
